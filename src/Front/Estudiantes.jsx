...
// Estilos
const sharedTableCellClasses = 'px-6 py-4 whitespace-nowrap text-sm text-zinc-900 dark:text-zinc-100';
const sharedTableHeaderClasses = 'px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider';
...
const TableHeader = () => {
  return (
    <thead className="bg-zinc-50 dark:bg-zinc-800">
      <tr>
        {["Legajo", "Nombre", "Apellido", "Email", "Telefono", "Detalles"].map(header => (
...
const StudentRow = ({ alumnos, onInfoClick }) => {
  return (
    <tr>
      <td className={`${sharedTableCellClasses} font-medium`}>{alumnos.legajo}</td>
      <td className={sharedTableCellClasses}>{alumnos.nombre}</td>
      <td className={sharedTableCellClasses}>{alumnos.apellido}</td>
      <td className={sharedTableCellClasses}>{alumnos.email}</td>
      <td className={sharedTableCellClasses}>{alumnos.telefono}</td>
      <td className={`${sharedTableCellClasses} text-right`}>
        <button
          ...
          onClick={() => onInfoClick(alumnos.legajo)}
        >
          Info
        </button>
      </td>
...
const Estudiantes = () => {
  ...
  const showStudentInfo = legajo => {
    alert(`Clickeaste al: ${legajo}`);
  };

  return (
    <div className="min-h-screen bg-zinc-100 dark:bg-zinc-900 p-4">
      <div className="container mx-auto">
        <h1 className="text-2xl font-bold mb-4 text-zinc-900 dark:text-zinc-100">Students List</h1>
        <div className="shadow-md overflow-hidden border-b border-zinc-200 dark:border-zinc-700 sm:rounded-lg">
          <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
            ...
            <tbody className="bg-white dark:bg-zinc-800 divide-y divide-zinc-200 dark:divide-zinc-700">
              {alumnos.map(alumnos => (
                <StudentRow key={alumnos.legajo} alumnos={alumnos} onInfoClick={showStudentInfo} />
              ))}
            </tbody>
          </table>
        </div>
